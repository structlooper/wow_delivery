...
class OrderAPIController extends Controller
{
    ...
    private function stripPayment(Request $request)
    {
        ...
        try {
            ...
            if ($stripeToken->created > 0) {
                ...
                DB::table('orders')->where('id',$order->id)->update([ 'tip_amount' => $request->tip_amount]);
    ...
    public function update($id, Request $request)
    {
        ...
        $order_details = DB::table('orders')->where('id',$id)->get();
        if($order_details->order_status_id == 5 and $orders_details->driver_id  != null){
            $tip_trans = floatval($orders_details->tip_amount);
            $driver_id = $orders_details->driver_id;
            DB::table('drivers')->where('id',$driver_id)->update(['earnings' => $tip_amount ]);
        }
